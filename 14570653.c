...
void caixeiroAux(Grafo*g, int atual, int numVisitados){
  ...
  PONT vizinho = listaDeVizinhos(g, atual);
  while(vizinho != NULL){
    if(!visitado[vizinho->vertice]){
      visitado[vizinho->vertice]=true;
      Peso valorAntigo = valorAtual;
      valorAtual= valorAtual + vizinho->peso;
      cicloAtual[numVisitados] = vizinho->vertice;
      caixeiroAux(g, vizinho->vertice, numVisitados+1);
      visitado[vizinho->vertice]= false;
      valorAtual= valorAntigo;
    }else if(vizinho->vertice == 0 && numVisitados==g->numVertices){
      valorAtual+= vizinho->peso; 
      if(melhorValor > valorAtual) {
        memcpy(melhorCiclo, cicloAtual, sizeof(int) * g->numVertices);
